...
def test_replay_buffer_sample_on_wrapped_full_buffer():
    # Regression: once the buffer wraps full, `pos` resets to 0 while the
    # number of valid rows is `buffer_size`.  `sample` used to size its
    # probability vector by `pos`, so a full buffer produced a zero-length
    # `prob` and np.random.choice raised "'a' and 'p' must have same size".
    env = make_vec_env(DummyEnv)
    buffer = ReplayBuffer(10, env.observation_space, env.action_space, device="cpu")
    env.reset()
    obs = env.reset()
    for _ in range(15):  # > buffer_size -> wraps to full, pos reset to 0
        action = env.action_space.sample()
        next_obs, reward, done, info = env.step(action)
        buffer.add(obs, next_obs, action, reward, done, info)
        obs = next_obs
    assert buffer.full
    assert buffer.pos == 5  # 15 adds: 10 fill the buffer, 5 wrapped
    sample = buffer.sample(5)
    assert sample.observations.shape[0] == 5
