...
class BaseBuffer(ABC):
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None):
        ...
        # `pos` resets to 0 once the buffer wraps full, so the probability
        # vector must be sized by `upper_bound`, not `self.pos`; otherwise a
        # full buffer yields a zero-length `prob` and np.random.choice raises
        # "'a' and 'p' must have same size" on every sample.
        if self.offline_mix_ratio > 0:
            if self.offline_steps > 0 and self.pos > 2*self.offline_steps:
                prob = np.ones(upper_bound)
                ...
                prob = np.ones(upper_bound) / upper_bound
        else:
            prob = np.ones(upper_bound) / upper_bound
        prob = prob / prob.sum()
